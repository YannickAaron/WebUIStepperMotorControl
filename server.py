...
Stepper.cleanAllGPIO()

motor = []

motor.append(Stepper.stepper([13, 11, 15, 37])) #step dir enable endstop
motor[0].isRunner()

motor.append(Stepper.stepper([10, 12, 8, 37])) #step dir enable endstop
motor[1].isRunner()
...
@sio.on('SetRunner')
async def print_message(sid, speedX, speedY):
    ## When we receive a new event of type
    ## 'message' through a socket.io connection
    ## we print the socket ID and the message
    print("Socket ID: " , sid)
    print('SetRunner MOTOR', speedX, speedY)
    motor[0].setRunner(int(speedX))
    motor[1].setRunner(int(speedY))
